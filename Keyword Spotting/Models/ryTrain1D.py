...
def get_all_fearure(all_x, batch_size= 1000):
    ...
    while i < x.shape[0]:
        ...
        print("Dimensión de los coeficientes de los espectrogramas de mel",X.shape)
...
from tensorflow.keras.layers import Conv1D, MaxPooling1D,SeparableConv1D,GlobalAveragePooling1D
...
h= Conv1D(40,kernel_size=101,strides=8, padding='same')(h)
h=tf.math.log(abs(h)+1)
h= BatchNormalization()(h)
h= AveragePooling1D(2)(h)

h= SeparableConv1D(160,kernel_size=25,strides=2, activation='relu', padding='same')(h)
h=BatchNormalization()(h)
h= AveragePooling1D(2, padding='same')(h)
h= SpatialDropout1D(0.1)(h)
 #Bloques de convolución DS-CONV-S
h= SeparableConv1D(160,kernel_size=9,strides=1, activation='relu', padding='same')(h)
h=BatchNormalization()(h)
h= AveragePooling1D(2, padding='same')(h)
h= SpatialDropout1D(0.1)(h)
