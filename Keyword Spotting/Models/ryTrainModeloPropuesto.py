...
fnModel= 'modelo_final_audios_new.hdf5'
...
def ryFeature(x,
    ...
    mfccs= tf.signal.mfccs_from_log_mel_spectrograms(log_mel_spectrograms)[..., :mfcc_dim]
    #print(mfccs.shape)
...
h= Conv2D(64,(15,12),strides=(2,2),activation=logarithm_activation,padding='same')(h)
...
#La quinta etapa es experimental y en esta no se quiere perder tanta información
h=SeparableConv2D(64,(3,3),strides=1,activation='relu',padding='same')(h)
h=BatchNormalization()(h)
h=AveragePooling2D(2,padding='same')(h)
h=SpatialDropout2D(0.6)(h)

#h=SeparableConv2D(13,3,strides=1,activation='relu')(h)
#h=BatchNormalization()(h)
#h=AveragePooling2D(2,padding='same')(h)
#h=SpatialDropout2D(0.1)(h)
GlobalAveragePooling2D()(h)
#h=Conv1D(1,1,activation='relu')(h)
...
h= m.fit(X_train, y_train,
        ...
        epochs=    4,#4500 #5000 épocas para el entrenamiento de las redes neuronales
...
tf.keras.utils.plot_model(m, to_file='model.png', show_shapes=True, show_layer_names=False)
